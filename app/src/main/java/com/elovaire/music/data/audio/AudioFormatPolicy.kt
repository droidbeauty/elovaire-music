...
internal object AudioFormatPolicy {
    val capabilities = listOf(
        ...
        AudioFormatCapability(AudioContainerFormat.Mp4Audio, "M4A/MP4 Audio", setOf("m4a", "mp4"), setOf("audio/mp4", "audio/mp4a-latm", "video/mp4"), PlaybackSupport.Supported, MetadataReadSupport.Strong, TagWriteSupport.Safe, true, "MP4 files must contain audio and no video track."),
        ...
        AudioFormatCapability(AudioContainerFormat.OggVorbis, "OGG", setOf("ogg"), setOf("audio/ogg", "application/ogg", "audio/vorbis"), PlaybackSupport.Supported, MetadataReadSupport.Partial, TagWriteSupport.Partial, false, "Codec is detected before playback; writes remain disabled."),
    ...
    fun capabilityFor(format: AudioContainerFormat): AudioFormatCapability? {
        val normalized = when (format) {
            AudioContainerFormat.OggOpus -> AudioContainerFormat.OggVorbis
            else -> format
        }
        return capabilities.firstOrNull { it.format == normalized }
    }
    ...
    fun resolveContainer(
        ...
        if (ext == "ogg" && codec == "audio/opus") return AudioContainerFormat.OggOpus
        return when (ext) {
            "mp3" -> AudioContainerFormat.Mp3
            "m4a", "mp4" -> AudioContainerFormat.Mp4Audio
            "aac" -> AudioContainerFormat.AacAdts
            "flac" -> AudioContainerFormat.Flac
            "wav" -> AudioContainerFormat.Wav
            "ogg" -> AudioContainerFormat.OggVorbis
    ...
    fun displayName(
        ...
        return when (container) {
            ...
            AudioContainerFormat.Mp4Audio -> if (extension.equals("m4a", true)) "M4A" else "MP4 Audio"
            AudioContainerFormat.AacAdts -> "AAC"
            AudioContainerFormat.Flac -> "FLAC"
            AudioContainerFormat.Wav -> "WAV"
            AudioContainerFormat.OggVorbis -> "OGG"
    ...
    private fun isCodecAllowed(container: AudioContainerFormat, codecMimeType: String?): Boolean {
        ...
        return when (container) {
            ...
            AudioContainerFormat.Mp4Audio -> codec in setOf("audio/mp4a-latm", "audio/aac", "audio/mpeg", "audio/opus")
